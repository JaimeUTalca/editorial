create or replace PACKAGE BODY          "PKG_INTEGRA_UTAL" IS
    ...
    g_sistema_sapdv   VARCHAR2(100) := 'http://sappiutalca:piutalca2016@sappodev.utalca.cl:51000';
    g_sistema_sapprod VARCHAR2(100) := 'http://sappiutalca:piutalca2016@sappoprod.utalca.cl:53000';
  ...
  -- qa v_respuesta := call_url_p('http://sappiutalca:piutalca2016@sappoqa.utalca.cl:52000/RESTAdapter/FISD01/INT_SAP02', v_json);
    ...
    PROCEDURE json_libros (
        p_prod_codigo NUMBER DEFAULT NULL
    ) IS
    ...
    BEGIN
        ...
        ;
        v_sql := 'select a.prod_codigo,a.prod_nombre,a.prod_descripcion,a.prod_precio,a.prod_imagen,a.prod_estado,b.libr_isbn,b.libr_agno,b.libr_num_paginas,'
                 || v_autores
                 || ','
                 || v_coleccion
...
where pove_libros_autores.auto_codigo=pove_autor.auto_codigo and prod_codigo=a.prod_codigo) as autores
     FROM vec_cob03.pove_producto a where  a.prod_precio is not null;
...
');
    ...
    FUNCTION int_leg05_json_cyp_fica (
        p_codigo_cli       IN VARCHAR2,
...
--
      ...
      --cursor de deudas en tabla temporal, agrupada por tipo
        ...
        SELECT DISTINCT
            pade_tipo_documento
        FROM
            vec_cob01.pop_pagos_detalle_temp_matri a
        WHERE
                pa_rut = p_idcliente
        ...
        IF p_ret = 'S' THEN
            ...
            FOR reg_grupo IN c_deudas_actuales(p_idcliente) LOOP
                BEGIN
                    ...
                    INTO
                        ...
                        pl_sub_operacion_op
                    FROM
                        operacion_sub_operacion_sap
                    WHERE
                        clase_documento_sap = reg_grupo.pade_tipo_documento;
                    ...
                    FOR reg IN c_deudas_actuales_detalle(reg_grupo.pade_tipo_documento) LOOP
                        ...
                        IF v_contador = 0 THEN
                            IF sysdate > TO_DATE ( reg.pade_fec_vencimiento, 'YYYYMMDD' ) THEN
                                v_fecha_documento := TO_DATE ( reg.pade_fec_vencimiento, 'YYYYMMDD' );
                            ELSE
                        ...
                        v_line := v_line
                              ...
                              "Fecha_documento": "'
                               ...
                               "Descripcion": "'
                                  ...
                                  || v_contador
                                  || '@'
                                  || v_convenio --reg.pade_observacion
                                  || '",
                               "Elemento_PEP": "",
                               "Pagar": "'
                                  || v_paga
         ...
         /*Fin llamada a servicio SAP*/
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            BEGIN
                                ...
                                l_data_json :=
                                    JSON(
                                        l_resp_json.get('Resp')
                                    );
                                p_ret := utsap001.pkg_integra_utal.lee_json(l_data_json, 'TYPE');
                                p_msg := utsap001.pkg_integra_utal.lee_json(l_data_json, 'MESSAGE');
                                IF p_ret = 'S' THEN
                                    UPDATE vec_cob01.jf_regulariza_2018_tmp
                                    SET
                                        procesado = 'S',
                ...
                IF l_data_json IS NULL THEN
                    p_ret := 'E';
                    p_msg := 'Error en el formato de la respuesta 2 : ' || v_respuesta;
                END IF;
    ...
    FUNCTION int_leg05_fica_crea_mat_sap (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2,
                --Salida estado si tiene error en oracle S (Success) E (Error)
...
--
                    ...
                    EXCEPTION
                        WHEN OTHERS THEN
                            ...
                            EXCEPTION
                                WHEN OTHERS THEN
                                    p_ret := 'E';
                                    p_msg := 'Error en el formato de la respuesta 1 : ' || sqlerrm;
                                    l_data_json := NULL;
                                    UPDATE vec_cob01.cc_pagos_banco_sap_matri
                                    SET
                                        codigo_msg_sap = p_ret,
    ...
    FUNCTION int_leg05_fica_crea_mat_grat (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2,
                --Salida estado si tiene error en oracle S (Success) E (Error)
...
--
    ...
    PROCEDURE int_leg05_llama_matri_sap (
        p_banco IN VARCHAR2
    ) IS
 ...
 --v_respuesta varchar2(32000);
        ...
        FROM
            (
                ...
                FROM
                    vec_cob01.cc_pagos_banco_sap_matri
                WHERE
                        'SANTANDER' = c_banco
                    AND ( estado_sap = ''
                          OR estado_sap IS NULL )
        ...
        FROM
            (
                SELECT
                    ...
                    COUNT(DISTINCT rut)        cant_rut
                FROM
                    vec_cob01.cc_pagos_bci_caja_sap_matri
                WHERE
                        'BCI' = c_banco
                    AND ( estado_sap = ''
                          OR estado_sap IS NULL )
    ...
    BEGIN
        FOR reg IN alumnos_matri(p_banco) LOOP
--CREACIÓN DE CLIENTES BANCO DE MATRICULAS
            BEGIN
                ...
                WHERE
                        pa_nro_operacion = reg.nro_operacion
                    AND pa_rut = reg.rut
                    AND ROWNUM = 1;
...
--CREACION DE CLIENTE
       ...
       --REGISTRAMOS EL LOG DE LA CREACION
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    INSERT INTO vec_cob02.log_error (
                        correlativo,
                        codigo_error,
                        mensaje_error,
                        fecha,
                    ...
                    COMMIT;
                    INSERT INTO vec_cob02.log_crea_cliente (
                        tipo_interlocutor,
                        cli_rut,
                        alu_nombres,
                        cli_apellidos,
     ...
     /*llama a la función int_sap11_json y en la variable l_cli_json recibe el json del data*/
            BEGIN
                IF ( reg.estado_sap = '' OR reg.estado_sap IS NULL ) THEN
                    l_cli_json := int_leg05_fica_crea_mat_sap(reg.rut, reg.nro_operacion, v_ret, v_msg);
                END IF;
            EXCEPTION
                WHEN OTHERS THEN
                    v_error := sqlerrm || dbms_utility.format_error_backtrace;
                    INSERT INTO log_error_deudas_sap (
                        codigo_error,
                        mensaje_error,
                        fecha,
                        mensaje_sap
  ...
  /*imprime estructura json por serpara y se revisa formato*/
    ...
    PROCEDURE reprocesar_pago_banco (
        p_banco     IN VARCHAR2,
        p_rut       VARCHAR2,
...
--p_fecha_documento      date;
        ...
        SELECT DISTINCT
            ...
            a.msg_sap
        FROM
            vec_cob01.cc_pagos_bci_caja_sap a
        WHERE
                estado_sap = 'E'
            AND rut = p_rut
        ...
        SELECT DISTINCT
            ...
            a.msg_sap
        FROM
            vec_cob01.cc_pagos_banco_sap a
        WHERE
                estado_sap = 'E'
            AND rut = p_rut
    ...
    BEGIN
        htp.p('<STRONG>REPROCESANDO CUPON '
              || p_rut
              || ' RUT '
              || p_nro_cupon
              || ' BANCO '
        ...
        IF p_ret = 'S' THEN
            ...
            v_json_fin := '';
            v_json := '';
            FOR reg IN matri_bci LOOP
                p_ret := 'S';
                p_idcliente := reg.rut;
                IF p_banco = 'BCI' THEN
                    p_tipo_documento := 'Y4';
                END IF;
                IF p_banco = 'SANTANDER' THEN
                    p_tipo_documento := 'Y3';
                END IF;
                ...
                v_json := v_respuesta;
                htp.p('<STRONG>LLAMADA JSON:</STRONG><BR>'
                      || v_json
                      || '<BR><BR>');
    ...
    PROCEDURE pago_banco_bci_masivo IS
...
(rut='18176989' and correlativo_id2=309840)
            ...
            AND EXISTS (
                SELECT
                    1
                FROM
                    vec_cob01.cc_pagos_bci_caja_sap_matri b
                WHERE
                        a.rut = b.rut
 ...
 --v_respuesta varchar2(32000);
    ...
    BEGIN
        FOR reg IN alumnos_matri(p_banco) LOOP
            v_rut := reg.rut;
            l_cli_json := utsap001.pkg_integra_utal.int_sap02_json_fica(reg.rut, '', '', '', '',
                                                                       '', v_ret, v_msg);

            IF l_cli_json IS NOT NULL THEN
                FOR i IN 1..l_cli_json.count LOOP
                    v_tipo_documento := utsap001.pkg_integra_utal.lee_json(
                                                                          JSON(
                                                                              l_cli_json.get(i)
                                                                          ), 'Tipo_Documento');

                    p_documento := utsap001.pkg_integra_utal.lee_json(
                                                                     JSON(
                                                                         l_cli_json.get(i)
                                                                     ), 'Documento');

                    v_fecha_vencimiento := utsap001.pkg_integra_utal.lee_json(
                                                                             JSON(
                                                                                 l_cli_json.get(i)
                                                                             ), 'Fecha_Vencimiento');
                    ...
                    BEGIN
                        IF ( p_banco = 'BCI' ) THEN
                            UPDATE vec_cob01.cc_pagos_bci_caja_sap
                            SET
                                documento_sap = TRIM(substr(p_documento,
                                                            3,
                                                            length(p_documento))),
                                nro_documento = TRIM(substr(p_documento,
                                                            3,
                                                            length(p_documento)))
                            WHERE
                                    rut = reg.rut
                                AND correlativo_id2 = reg.nro_operacion
                                AND TRIM(substr(descripcion, 5, 3)) = v_tipo_documento
                ...
                END LOOP;
            ELSE
                l_cli_json_data := utsap001.pkg_integra_utal.int_sap02_json_data(v_rut, '', '', '', '',
                                                                                '', v_ret, v_msg);

                p_documento := utsap001.pkg_integra_utal.lee_json(l_cli_json_data, 'Documento');
            END IF;
...
--
    ...
    FUNCTION int_leg05_sd_titulacion (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2,
                --Salida estado si tiene error en oracle S (Success) E (Error)
            ...
            IF p_ret = 'S' THEN
               ...
               --obtenemos el material de la tabla de productos
                        BEGIN
                            ...
                            INTO v_material
                            FROM
                                vec_cob01.pop_productos
                            WHERE
                                prod_id = reg.pade_prod_id;
    ...
    PROCEDURE llama_crea_paga_venta (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2
    ) IS
        ...
        p_msg   VARCHAR2(5000);
    BEGIN
        llamada := pkg_integra_utal.int_leg05_sd_venta(p_idcliente, p_num_op, p_ret, p_msg);
        llamada.htp;
        htp.p('<br><br>'
              || p_ret
              || '-mensaje: '
    ...
    FUNCTION f_calculadespacho_sap (
        p_venta_codigo NUMBER
    ) RETURN NUMBER IS
    ...
    BEGIN
        BEGIN
            ...
            FROM
                vec_cob03.pove_venta_detalle
            WHERE
                vent_codigo = p_venta_codigo;

            SELECT
                decode(a.clie_retiro,
                       'S',
                       0,
                       vec_cob03.venta_online.f_costo_envio(tade_codigo, cire_codigo, v_cantidad_total)) AS valor_despacho
            INTO v_total_despacho
            FROM
                     vec_cob03.pove_cliente a
                INNER JOIN vec_cob03.pove_pais   i ON i.pais_codigo = a.pais_codigo
                INNER JOIN vec_cob03.pove_region r ON r.regi_codigo = a.regi_codigo
                INNER JOIN vec_cob03.pove_ciudad c ON c.ciud_codigo = a.ciud_codigo
            WHERE
                clie_codigo = p_venta_codigo;
...
/*llamada Creacion de deuda por ventas leg05*/
    ...
    BEGIN
     /*llama a la función int_sap11_json y en la variable l_cli_json recibe el json del data*/
        BEGIN
            l_cli_json := int_leg05_json_fica(p_codigo_cli, p_tipo_documento, p_fecha_documento, p_num_cuponera, p_importe,
                                             p_carrera, p_num_matricula, p_centro_beneficio, p_operacion, p_sub_operacion,
                                             p_descripcion, v_ret, v_msg);
        EXCEPTION
            WHEN OTHERS THEN
...
/*http://condor2.utalca.cl/pls/sap_test/pkg_integra_utal.int_leg05_fica?p_codigo_cli=124140692&p_fecha_documento=20170101&p_tipo_documento=MA&p_num_cuponera=1&p_importe=115555&p_carrera=04&p_num_matricula=97401027&p_centro_beneficio=FCA310001&p_operacion=&p_sub_operacion=&p_descripcion=ARANCEL*/

    FUNCTION int_leg05_json_fica (
        p_codigo_cli       IN VARCHAR2,
        p_tipo_documento   IN VARCHAR2,
        p_fecha_documento  IN VARCHAR2,
        p_num_cuponera     IN VARCHAR2,
   ...
   /* Recupera Token*/
        BEGIN
            SELECT
                utal_dti.p_encrypt_utal.encrypt_ssn_sap(g_clave || to_char(sysdate, 'dd/mm/yyyy hh24:mi:ss')) AS dato_encriptado
            INTO v_token
            FROM
                dual;
   ...
   /* if p_ret = 'S' then
    ...
    BEGIN
        ...
        utl_http.set_header(v_req, 'token', '1b2b594f40de99626a2328f6bbba5ea9');
        --utl_http.write_raw(v_req, utl_raw.cast_to_raw(convert(p_json, 'UTF8', v_nls_characterset)));
    ...
    FUNCTION int_leg05_sd_portal_integral (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2, --Salida estado si tiene error en oracle S (Success) E (Error)
        p_msg       OUT VARCHAR2 --mensaje de error
        ...
        v_json_cabecera         VARCHAR2(32767); -- Para almacenar el JSON acumulado
        v_test                  VARCHAR2(100) := NULL;
        ...
        SELECT
            ...
            pade_nro_carrera,
            pade_tipo_documento,
            e.clase_documento_sap
        FROM
                 vec_cob01.pop_pagos_detalle_temp_sap a
            JOIN vec_cob01.pip_productos2     b ON TO_CHAR(a.pade_prod_id) = b.codigo_sap
            JOIN utsap001.sap_categoria_canal c ON c.categoria = b.id_categoria
            JOIN vec_cob02.webpay_trasaccion  d ON TO_CHAR(d.id_sesion) = TO_CHAR(a.pa_nro_operacion) --se ajusta select para nuevo campo SAP DS 19/05/2025
            JOIN vec_cob02.tipo_pago_descripcion e ON d.tipo_pago = e.codigo_pago --se ajusta select para nuevo campo SAP DS 19/05/2025
        WHERE
        ...
        GROUP BY
            ...
            pade_nro_carrera,
            pade_tipo_documento,
            e.clase_documento_sap; --Se agrega clase de documento SAP para nuevo campo de intregación SAP DS 19/05/2025

        CURSOR c_items (
            p_tipo_doc      VARCHAR2,
            p_canal_detalle VARCHAR2
        ) IS
        SELECT
            *
        FROM
        ...
        WHERE
            ...
            AND a.pa_nro_operacion = p_num_op
            AND TO_CHAR(a.pade_prod_id) = b.codigo_sap
            AND c.categoria = b.id_categoria
            AND c.canal = p_canal_detalle;
        ...
        BEGIN
            ...
            IF p_ret = 'S' THEN
                FOR reg_grupo IN c_cabecera LOOP
                    IF v_canal_anterior IS NULL OR v_canal_anterior != reg_grupo.canal THEN
                        v_line := '{
                                    "TOKEN": "'
                                  || v_token
                                  || '",
                    ...
                    IF v_canal_anterior_det IS NULL OR v_canal_anterior_det != reg_grupo.canal THEN
                        --DETALLE DE LOS PRODUCTOS
                        v_posicion_item := 10;
                        FOR reg IN c_items(reg_grupo.pade_tipo_documento, reg_grupo.canal) LOOP
                            v_line := ' "ORDER_ITEMS_IN": {
                                        "Posicion_documento": "'
                                      || v_posicion_item
                                      || '",
                            ...
                            --se agrega nuevo campo json para integración con SAP DS 19/05/2025
                            v_json := v_json || v_line;
                            v_line := '';
                            v_line := '"ORDER_PARTNERS": {
                                        "Funcion_interlocutor": "AG",
                                        "Numero_deudor": "'
                                      || reg.pa_rut
                                      || '",
                    ...
                    v_canal_anterior_det := reg_grupo.canal;
                    v_canal_anterior := reg_grupo.canal; --02
                    v_line := '';
                    v_line := '} }';
                    v_json := v_json || v_line;
...
--htp.p(v_json);
    ...
    FUNCTION int_leg05_sd_portal_xml (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2, --Salida estado si tiene error en oracle S (Success) E (Error)
        p_msg       OUT VARCHAR2 --mensaje de error
        ...
        WHERE
                a.pa_rut = p_idcliente
            AND a.pa_nro_operacion = p_num_op
            AND TO_CHAR(a.pade_prod_id) = b.codigo_sap -- SE AJUSTA PARA CODIGO_SAP EN PIP_PRODUCTOS2 30/08/2024 DS
            AND c.categoria = b.id_categoria;
    ...
    BEGIN
      ...
      /* Recupera Token*/
        BEGIN
            ...
            FOR reg_grupo IN c_deudas_actuales() LOOP
                v_posicion_item := 30;
                dbms_output.put_line('v_posicion_item ' || v_posicion_item);
            END LOOP;

            dbms_output.put_line('v_posicion_item 2 ' || v_posicion_item);
        END;

        RETURN l_data_json;
    END int_leg05_sd_portal_xml;

    FUNCTION int_leg05_json_sd (
...
';
   ...
   /* if p_ret = 'S' then
    ...
    PROCEDURE int_leg05_sd (
        p_clase_documento  IN VARCHAR2 DEFAULT NULL,
    ...
    BEGIN
     /*llama a la función int_sap11_json y en la variable l_cli_json recibe el json del data*/
        BEGIN
            l_cli_json := int_leg05_json_sd(p_clase_documento, p_canal, p_fecha_documento, p_matricula, p_codigo_carrera,
                                           p_numero_material, p_centro_beneficio, p_numero_deudor, p_monto, v_ret,
                                           v_msg);
        EXCEPTION
            WHEN OTHERS THEN
                htp.p(sqlerrm || dbms_utility.format_error_backtrace);
...
"Monto_total": "'||v_monto_total2||'" "Monto total del beneficio"
    ...
    FUNCTION int_leg05_json_fica_postgrado (
        ...
        SELECT
            ...
            moneda
        FROM
            vec_postgrado.cc_documentos
        WHERE
                codigo_cli = p_rut_alumno
   ...
   /* Fin Recupera Token*/
        ...
        IF p_ret = 'S' THEN
            ...
            BEGIN
                SELECT
                    COUNT(*),
                    trunc(last_day(sysdate)) - trunc(sysdate)
                INTO
                    pl_total_vencidas,
                    pl_dias_mes
                FROM
                ...
                IF pl_dias_mes < pl_total_vencidas THEN
                    pl_fecha_documento := to_char(sysdate - pl_total_vencidas, 'yyyymmdd');
                ELSE
            ...
            FOR r IN c_deuda_postgrados(p_conf_canal) LOOP
                BEGIN
                    ...
                    BEGIN
                        ...
                        FROM
                            operacion_sub_operacion_sap
                        WHERE
                            clase_documento_icon = p_numero_material;
                    ...
                    BEGIN
                        ...
                        FROM
                            (
                                SELECT
                                    utsap001.pkg_recursos.recupera_codigo_sap(2, 2, NULL, p_numero_material) sap_id
                                FROM
                                    dual
                            )
                        WHERE
                    ...
                    IF TO_NUMBER ( pl_fecha_vencimiento ) < TO_NUMBER ( pl_fecha_documento ) THEN
                        pl_fecha_vencimiento := pl_fecha_documento + r.cuota;
                    END IF;

                    v_line_deuda := ' "ZCLFICA_MF_CREADEUDA":{
                               "Codigo_cli": "'
                                    || p_rut_alumno
                                    || '",
                               "Tipo_documento": "'
                    ...
                    IF pl_add_coma > 0 THEN
                        dbms_lob.writeappend(v_line, length(',' || v_line_deuda), ',' || v_line_deuda);
                    ELSE
                        dbms_lob.writeappend(v_line, length(v_line_deuda), v_line_deuda);
                    END IF;
            ...
            BEGIN
                ...
                v_respuesta := call_url_p_1(g_sistema_sap || '/RESTAdapter/SD001/INT_LEG05', v_line);
            EXCEPTION
                WHEN OTHERS THEN
                    p_ret := 'E';
                    p_msg := p_msg
            ...
            IF p_ret = 'S' THEN
               ...
               --SI EXISTE UNA CON ERROR SE ENVIA ERROR
                    v_ret := 'S';
                    p_msg := '';
                    FOR i IN 1..l_data_json_l.count LOOP
                        l_data_json :=
                            JSON(
                                l_data_json_l.get(1)
                            );
...
/*llamada crea un un cliente leg04*/
    PROCEDURE int_leg04_13_02_2017 (
        v_rut                      VARCHAR2,
...
"KALSK"          : "NO ESTA"
    ...
    BEGIN
        ...
        v_dato := v_dato
                  || utal_dti.p_encrypt_utal.decrypt_ssn(owa_util.get_cgi_env('HTTP_AUTHORIZATION'));
...
/*************************************************************************/
    ...
    PROCEDURE get_ganador_aniversario (
        p_premio IN VARCHAR2 DEFAULT NULL
    ) IS
    ...
    BEGIN
        ...
        IF p_premio IS NOT NULL THEN
            UPDATE evb_pedidos
            SET
                flag_premio = substr(p_premio, 1, 50)
            WHERE
        ...
        v_sql := 'SELECT decode(instr(nombre    , '' '' ) , 0 ,nombre    , substr(nombre   ,0, instr(nombre    , '' '' )-1 )) nombre ,
...
-- Devuelve carrera alumno
    PROCEDURE get_carrera_alumno (
        v_rut_est IN VARCHAR2
    ) IS
        v_sql VARCHAR2(5000);
    BEGIN
        v_sql := 'select pra_nombre
              from(
                    select *
...
-- f_int04
    PROCEDURE get_contratos_funcionario (
        v_rol_emp  IN VARCHAR2,
        v_vigencia IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_contrato_utsap where rol_emp='''
                 || v_rol_emp
                 || ''' and vigencia='''
                 || v_vigencia
                 || ''' ';
...
-- f_int05
    PROCEDURE get_estudios_funcionario (
        v_rol_emp IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where rol_emp='''
                 || v_rol_emp
                 || ''' ';
        get_json(v_sql);
    ...
    PROCEDURE get_sit_acad_estudiante (
        ...
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select alu_rut_n, alumno_pkg.sel_nombre(alu_rut_n) nombre,
...
and pra_codigo = '''
  ...
  "data" : {   "alumnos" : [ { "rut" : "10580692", "fecha" : "03-01-2006" },
    ...
    PROCEDURE get_situaciones_masiva (
        p_json IN VARCHAR2
    ) IS
 ...
 -- htp.p(v_1);
        v_respuesta := '';
        FOR i IN 1..p_json1.count LOOP
            l_data_json := json_list(p_json1.get(i));
            FOR j IN 1..l_data_json.count LOOP
                v_rut := lee_json(
                                 JSON(
                                     l_data_json.get(j)
                                 ), 'rut');
...
--htp.p(v_respuesta2);
    ...
    BEGIN
        v_sql := 'select alu_rut_n, alumno_pkg.sel_nombre(alu_rut_n) nombre, sit_f_resol as fecha,
...
where a.pra_codigo=programa_academico.pra_codigo';
        ...
        BEGIN
            ...
            FROM
                ...
                programa_academico
            WHERE
                a.pra_codigo = programa_academico.pra_codigo;
    ...
    PROCEDURE get_situacion_acad_fecha (
        v_rut_est IN VARCHAR2,
        v_fecha   IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
and  trunc(sit_f_resol) <= trunc(to_date('''
                 || v_fecha
                 || ''',''dd-mm_yyyy''))
order by pal_fecha_ingreso desc
...
--htp.p(v_sql);
    ...
    PROCEDURE get_deuda_biblioteca (
        p_id IN VARCHAR2 DEFAULT NULL
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from V_REM_DEUDAS_BIBLIOTECA db where  trim(db.id) = trim(nvl('''
                 || p_id
                 || ''' , db.id))  ';
--htp.p(v_sql);
    ...
    PROCEDURE get_situaciones_estudiante (
        v_rut_est IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select alu_rut_n, alumno_pkg.sel_nombre(alu_rut_n) nombre,pa.pra_codigo,p.pra_nombre,
...
--htp.p(v_sql);
    ...
    END;
-- f_int05
    PROCEDURE get_deuda_estudiante (
        v_rut_est        IN VARCHAR2,
    ...
    BEGIN
       /*SQL ORIGINAL*/
        v_sql := 'select trim(a.tipo_documento) as tipo_documento,fec_recepcion,c.moneda as tipo_moneda,a.documento,nvl(carrera,''0'') as carrera,
...
-- f_int06
    PROCEDURE get_prevision_funcionario (
        v_rol_emp  IN VARCHAR2,
        v_vigencia IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
-- deuda institucional
    PROCEDURE get_deuda_institucional (
        v_rut IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := ' select c.descripcion as tipo_de_credito,a.ano,a.matricula,decode(a.moneda,''02'',''04'',decode(a.moneda,''03'',''03'')) as moneda,
...
-- utm
    PROCEDURE get_indicador_utm IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := '  select valor
...
-- f_int07
    PROCEDURE get_licencias_funcionario (
        v_rol_emp IN VARCHAR2,
        v_agno    IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
-- f_int08
    PROCEDURE get_unidad_funcionario (
        v_rol_emp IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
--ejemplos  8024316 76129865
        v_sql := 'select Rut, Dvr,Nombre,Direccion,Comuna,Ciudad,Region,Telefono,Email,''RB-001'' Rubro from utalca.ct_entidad
where rut='''
                 || v_rut
                 || ''' ';
        get_json(v_sql);
...
-- Datos de la deuda
    ...
    PROCEDURE get_datos_deuda (
        v_rut IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
--ejemplos  15222778
        v_sql := 'select codigo_cli Rut, utalca.calcula_digito(codigo_cli) dv,
...
-- Datos de Biblioteca
    PROCEDURE get_datos_biblioteca (
        v_rut IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
/*indica que tipo de pago tiene asociado un pago de alumno webpay etc..*/
    PROCEDURE get_pago_convenios (
        v_tipo_convenio   IN VARCHAR2,
        v_nombre_convenio IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select count(*) cantidad
                  from pop_convenios
...
-- F2_int01
    PROCEDURE get_saldo_alumno (
        v_codigo_cli IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where codigo_cli='''
                 || v_codigo_cli
                 || ''' ';
        get_json(v_sql);
...
-- F2_int02
    PROCEDURE get_saldo_agno_alumno (
        v_codigo_cli IN VARCHAR2,
        v_ano        IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
-- F2_int04
    PROCEDURE get_datos_alumno (
        v_rut_est IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select alu_rut_n,alu_rut_v,alu_nombre from vac_estruc.alumno where alu_rut_n='''
                 || v_rut_est
                 || ''' ';
        get_json(v_sql);
...
-- F2_int05
    PROCEDURE get_precio_titulo_alumno (
        v_codigo_cli IN VARCHAR2,
        v_ano        IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
-- F3_int01
    PROCEDURE get_fondo_presupuesto (
        v_centro_costo IN VARCHAR2,
        v_cuenta       IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where Centro_costo='''
                 || v_centro_costo
                 || ''' and Cuenta='''
                 || v_cuenta
                 || ''' ';
...
-- F3_int02
    PROCEDURE get_licitacion_presupuesto (
        v_centro_costo IN VARCHAR2,
        v_cuenta       IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
...
-- F4_int01
    PROCEDURE get_tipo_doc_beneficios (
        v_tipo_documento IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where Tipo_documento='''
                 || v_tipo_documento
                 || ''' ';
        get_json(v_sql);
...
-- F4_int03
    PROCEDURE get_datos_doc_beneficios (
        v_tipo_documento IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
-- F5_int01
    PROCEDURE get_nombre_centro_postulacion (
        v_codigo_cli IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
-- F5_int02
    PROCEDURE get_datos_centro_postulacion (
        v_codigo_cli IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
-- F7_int01
    PROCEDURE get_rendicion_pendiente (
        v_codigo_prv IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where Codigo_prv='''
                 || v_codigo_prv
                 || ''' ';
        get_json(v_sql);
...
-- F8_int01
    PROCEDURE get_pagares_alumno (
        v_codigo_cli IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := ' select ano,carrera,matricula,vec_cob01.interes_fscu(ano, to_number(to_char(sysdate,''yyyy'')), '
...
-- F10_int01
    ...
    PROCEDURE get_web_token IS
        v_sql   VARCHAR2(2000);
        p_clave VARCHAR2(100) := 'SAPUTALCA';
    BEGIN
        v_sql := 'select utal_dti.p_encrypt_utal.encrypt_ssn_sap('''
                 || p_clave
    ...
    PROCEDURE desencriptar_web_token (
        p_token VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select utal_dti.p_encrypt_utal.decrypt_ssn('''
                 || p_token
...
-- F11_int01
    PROCEDURE get_datos_productos (
        v_codigo_producto IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
        v_sql := 'select * from v_rem_ficha_utsap where codigo_producto='''
                 || v_codigo_producto
                 || ''' ';
        get_json(v_sql);
...
-- f_int01
    PROCEDURE get_datos_funcionario (
        v_rol_emp IN VARCHAR2
    ) IS
        v_sql VARCHAR2(2000);
    BEGIN
...
-- f_int02
    FUNCTION get_nombre_funcionario (
        v_rol_emp IN VARCHAR2
    ) RETURN VARCHAR2 IS
        v_sql VARCHAR2(2000);
    BEGIN
...
var autorizacion="'
              || utal_dti.p_encrypt_utal.encrypt_ssn(to_char(sysdate, 'dd/mm/yyyy hh24:mi:ss'))
              || '";
$(document).ready(function(){
...
');
    ...
    PROCEDURE pago_inst_financieras (
        p_rut                NUMBER,
        p_banco              VARCHAR2,
        p_num_op             VARCHAR2,
...
*/
 ...
 -- utsap001.pkg_recursos.recupera_codigo_sap(1, 10, '', alu_comuna_origen_alu)
        ...
        IF ( p_banco = 'SANT' ) THEN
            vec_cob01.portaldepagos.deudas_vigentes_json(p_rut);

--FOR reg_grupo In c_deudas_actuales(p_rut)
  --             LOOP
            v_tipo_doc_sap := pkg_recursos.recupera_codigo_sap(2, 2, NULL, param_tipo_documento);
            FOR reg IN c_deudas_actuales_detalle(p_rut, p_num_op, v_tipo_doc_sap) LOOP
...
/*    select distinct pade_tipo_documento into v_tipo_doc
         ...
         and PA_NRO_OPERACION=p_num_op;*/
          ...
          --  v_fecha_registro:=p_fecha_registro;
                ...
                v_fecha_pago := TO_DATE ( to_char(TO_DATE(p_fecha_pago, 'DD-MON-YY'),
                                                  'DD-MON-YYYY'), 'DD-MM-YYYY' );--to_date(TO_CHAR(p_fecha_pago,'DD-MM-YYYY'),'DD-MM-YYYY');
         -- v_fecha_pago:=SUBSTR(p_fecha_pago,7,2)||'-'||SUBSTR(p_fecha_pago,5,2)||'-'||SUBSTR(p_fecha_pago,1,4);
                v_fecha_registro := TO_DATE ( to_char(TO_DATE(p_fecha_registro, 'DD-MON-YY'),
                                                      'DD-MON-YYYY'), 'DD-MM-YYYY' );--SUBSTR(p_fecha_registro,7,2)||'-'||SUBSTR(p_fecha_registro,5,2)||'-'||SUBSTR(p_fecha_registro,1,4);
            --p_fecha_documento:=SUBSTR(p_fecha_documento,7,2)||'-'||SUBSTR(p_fecha_documento,5,2)||'-'||SUBSTR(p_fecha_documento,1,4);
                p_fecha_vencimiento := TO_DATE ( substr('20171023', 7, 2)
                                                 || '-'
                                                 || substr('20171023', 5, 2)
                                                 || '-'
                                                 || substr('20171023', 1, 4), 'DD-MM-YYYY' );--TO_CHAR(TO_DATE(p_fecha_vencimiento,'DD-MON-YY'),'DD-MON-YYYY');
                ...
                COMMIT;
                BEGIN
                    INSERT INTO vec_cob01.cc_pagos_banco_sap (
                        nro_contrato,
                        rut,
                        ano,
                        cuota,
...
exception when others then
    ...
    */
    ...
    PROCEDURE pago_inst_financieras_resp (
        p_rut   NUMBER,
        p_banco VARCHAR2
                                /*p_NRO_DOCUMENTO varchar2,
                                p_ano number,
...
*/
    ...
    BEGIN
        IF ( p_banco = 'SANT' ) THEN
            BEGIN
                l_cli_json := int_sap02_json(p_rut, '', '', '', '',
                                            '', v_ret, v_msg);
            EXCEPTION
                WHEN OTHERS THEN
                    v_error := sqlerrm || dbms_utility.format_error_backtrace;
            ...
            FOR i IN 1..l_cli_json.count LOOP
              ...
              --  EXIT WHEN v_contador > 1;
                ...
                v_codigo_cli := TO_NUMBER ( utsap001.pkg_integra_utal.lee_json(
                    JSON(
                        l_cli_json.get(i)
                    ),
                    'Codigo_cli') );

                v_empresa := pkg_integra_utal.lee_json(
                                                      JSON(
                                                          l_cli_json.get(i)
                                                      ), 'Empresa');

                v_cuenta_contrato := pkg_integra_utal.lee_json(
                                                              JSON(
                                                                  l_cli_json.get(i)
                                                              ), 'Cuenta_Contrato');

                v_tipo_cuenta_contrato := pkg_integra_utal.lee_json(
                                                                   JSON(
                                                                       l_cli_json.get(i)
                                                                   ), 'Tipo_Cuenta_Contrato');

                v_objeto_contrato := pkg_integra_utal.lee_json(
                                                              JSON(
                                                                  l_cli_json.get(i)
                                                              ), 'Objeto_Contrato');

                v_clase_objeto_contrato := pkg_integra_utal.lee_json(
                                                                    JSON(
                                                                        l_cli_json.get(i)
                                                                    ), 'Clase_Objeto_Contrato');

                v_tipo_documento := pkg_integra_utal.lee_json(
                                                             JSON(
                                                                 l_cli_json.get(i)
                                                             ), 'Tipo_Documento');

                v_tipo_doc := substr(pkg_integra_utal.lee_json(
                                                              JSON(
                                                                  l_cli_json.get(i)
                                                              ), 'Tipo_Documento'), 1, instr(v_tipo_documento, '-') - 1);
--                      v_tipo_doc:= PKG_RECURSOS.RECUPERA_CODIGO_ICON(2,2,'',v_tipo_doc); se debe usar en el deuda SD

                v_documento := pkg_integra_utal.lee_json(
                                                        JSON(
                                                            l_cli_json.get(i)
                                                        ), 'Documento');

                v_cuota := pkg_integra_utal.lee_json(
                                                    JSON(
                                                        l_cli_json.get(i)
                                                    ), 'Cuota');

                v_posicion_interes := pkg_integra_utal.lee_json(
                                                               JSON(
                                                                   l_cli_json.get(i)
                                                               ), 'Posicion_Interes');

                v_fecha_vencimiento := pkg_integra_utal.lee_json(
                                                                JSON(
                                                                    l_cli_json.get(i)
                                                                ), 'Fecha_Vencimiento');

                v_fecha_documento := lee_json(
                                             JSON(
                                                 l_cli_json.get(i)
                                             ), 'Fecha_Focumento');
                v_total := pkg_integra_utal.lee_json(
                                                    JSON(
                                                        l_cli_json.get(i)
                                                    ), 'Total');
                ...
                v_total_local := replace(pkg_integra_utal.lee_json(
                                                                  JSON(
                                                                      l_cli_json.get(i)
                                                                  ), 'Total_Local'), '.', '');
                ...
                v_saldo := replace(pkg_integra_utal.lee_json(
                                                            JSON(
                                                                l_cli_json.get(i)
                                                            ), 'Saldo'), '.', '');

                v_saldo := replace(v_saldo, ',', '');
                v_saldo_local := replace(pkg_integra_utal.lee_json(
                                                                  JSON(
                                                                      l_cli_json.get(i)
                                                                  ), 'Saldo_Local'), '.', '');

                v_carrera := substr(pkg_integra_utal.lee_json(
                                                             JSON(
                                                                 l_cli_json.get(i)
                                                             ), 'Carrera'), 1, 2);

                v_intereses_local := replace(pkg_integra_utal.lee_json(
                                                                      JSON(
                                                                          l_cli_json.get(i)
                                                                      ), 'Intereses_Local'), '.', '');

                v_interes := replace(pkg_integra_utal.lee_json(
                                                              JSON(
                                                                  l_cli_json.get(i)
                                                              ), 'Interes'), '.', '');

                v_valor_de_cambio := replace(pkg_integra_utal.lee_json(
                                                                      JSON(
                                                                          l_cli_json.get(i)
                                                                      ), 'Valor_de_Cambio'), '.', '');

                v_moneda := pkg_integra_utal.lee_json(
                                                     JSON(
                                                         l_cli_json.get(i)
                                                     ), 'Moneda');

                v_nro_matricula := pkg_integra_utal.lee_json(
                                                            JSON(
                                                                l_cli_json.get(i)
                                                            ), 'Nro_Matricula');
...
END IF;*/
                ...
                v_fecha_venc := substr(v_fecha_vencimiento, 7, 2)
                                || '-'
                                || substr(v_fecha_vencimiento, 5, 2)
                                || '-'
                                || substr(v_fecha_vencimiento, 1, 4);

                v_fecha_doc := substr(v_fecha_documento, 7, 2)
                               || '-'
                               || substr(v_fecha_documento, 5, 2)
                               || '-'
                               || substr(v_fecha_documento, 1, 4);

                FOR j IN corr_pagos_banco LOOP
...
*/
    ...
    FUNCTION encripta_sap RETURN VARCHAR2 IS
        l_retorno VARCHAR2(1000);
    BEGIN
        l_retorno := utal_dti.p_encrypt_utal.encrypt_ssn_sap(g_clave || to_char(sysdate, 'dd/mm/yyyy hh24:mi:ss'));

        RETURN l_retorno;
    END encripta_sap;

    FUNCTION desencripta_sap (
        p_valor VARCHAR2
    ) RETURN VARCHAR2 IS
        l_retorno VARCHAR2(1000);
    BEGIN
...
END call_url_p;*/
    ...
    BEGIN
      ...
      --v_largo:=utl_raw.length(utl_raw.convert(utl_raw.cast_to_raw( p_json),'american_america.al32utf8',v_NLS_CHARACTERSET));
        v_largo := utl_raw.length(utl_raw.cast_to_raw(convert(p_json, 'UTF8', v_nls_characterset)));
    ...
    BEGIN
     ...
     -- htp.p('hola');
        v_largo := utl_raw.length(cast_to_raw_dti(convert(p_json, 'UTF8', v_nls_characterset)));
        ...
        utl_http.set_header(v_req, 'content-type', 'application/json; charset=utf-8');
        utl_http.set_header(v_req, 'Content-Length', v_largo);
        utl_http.write_raw(v_req, cast_to_raw_dti(convert(p_json, 'UTF8', v_nls_characterset)));
        UTL_HTTP.SET_HEADER (v_req,
                           'Authorization',
                           'Basic ' || g_clave_sap_pipo);
        v_resp := utl_http.get_response(v_req);
      ...
      --htp.p(v_url);
        ...
        ELSE
            ...
            WHILE ( offset < req_length ) LOOP
                dbms_lob.read(p_json, amount, offset, buffer);
    ...
    FUNCTION int_sap03_json (
        p_agno_ejercicio VARCHAR2, --parametro consulta
   ...
   /* Fin Recupera Token*/
    ...
    --v_token := v_token||'*';
      ...
      /*Fin Json de entrada */
            ...
            /*  sappodev:desarrollo  puerto 51000
                sappoqa:testing      puerto 52000
            */
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/FM001/INT_SAP03', v_json);
    ...
    BEGIN
     /*llama a la función int_sapxx_json y en la variable l_cli_json recibe el json del data*/
        l_cli_json := int_sap03_json(p_agno_ejercicio, p_centro, p_proyecto, p_orden_co, p_cuenta,
                                    v_ret, v_msg);
  ...
  /*imprime estructura json por serpara y se revisa formato*/
        IF v_ret = 'S' THEN
            htp.p('Centro_Gestor:'
                  ...
                  || '<br>');
            htp.p('Pospre:'
                  || lee_json(l_cli_json, 'Pospre')
                  || '<br>');
            htp.p('Monto:'
                  || lee_json(l_cli_json, 'Monto')
                  || '<br>');
            htp.p('<br>');
...
*/
    ...
    FUNCTION int_sap09_json (
        p_material VARCHAR2, --parametro consulta
        p_centro   VARCHAR2,  --parametro consulta
        p_ret      OUT VARCHAR2, --Salida estado si tiene error en oracle S (Success) E (Error)
        p_msg      OUT VARCHAR2 --mensaje de error
    ) RETURN json IS
   ...
   /* Fin Recupera Token*/
      ...
      /*Fin Json de entrada*/
          ...
          --   v_respuesta := call_url_p('http://sappiutalca:piutalca2016@sappoqa.utalca.cl:52000/RESTAdapter/MM004/INT_SAP09', v_json);
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/MM004/INT_SAP09', v_json);
    ...
    BEGIN
     /*llama a la función int_sapxx_json y en la variable l_cli_json recibe el json del data*/
        l_cli_json := int_sap09_json(p_material, p_centro, v_ret, v_msg);
        htp.p(v_ret || '<br>');

  /*imprime estructura json por serpara y se revisa formato*/
        IF v_ret = 'S' THEN
            htp.p('Material:'
                  || lee_json(l_cli_json, 'Material')
                  || '<br>');
            htp.p('Cantidad:'
                  || lee_json(l_cli_json, 'Cantidad')
                  || '<br>');
            htp.p('<br>');
        ELSE
    /*imprime mensaje de error*/
   ...
   /* Fin Recupera Token*/
        ...
        IF p_ret = 'S' THEN
         ...
         /*Orden_CO siempre va vacío*/
            ...
            v_contador_centro_costo := pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'CONTADOR_CENTRO_COSTO', 1);
            v_fecha1 := pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'FECHA_LICITACION');
       --pkg_integra_utal.LEERDATOTABLA_WF(itemtype,itemkey,'FECHA_LICITACION');

            v_fecha_licitacion := to_char(TO_DATE(v_fecha1, 'dd-mm-yyyy'), 'yyyy')
            ...
            v_json := '{
                ...
                "data": {
             ';
            FOR i IN 1..v_contador_centro_costo LOOP
                v_centro_costo_sap := pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'TXT_C_RESPONSABILIDAD', i);
                SELECT DISTINCT
                    tipo_centro
                INTO v_tipo_centro
                FROM
                ...
                v_json := v_json
                          ...
                          || '",
                                               "Nombre_Solicitante": "'
                          || pkg_integra_utal.leerdatotabla_wf('WF_LICI', itemkey, 'USR_UNIDAD')
                          || '",
                                               "Fecha_Creacion": "'
                          || v_fecha_licitacion
                          || '",
                                               "Texto_Breve": "'
                          || pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'TXT_NOMBRE_LICITACION')
                          || '",
                                               "Material": "",
                                               "Centro": "UT01",
                                               "Numero_Necesidad": "'
                          ...
                          || '",
                                               "Precio": "'
                          || pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'TXT_NUEVO_MONTO', i)
                          || '",
                                               "Imputacion": "'
                          || v_tipo_centro_sap
                          || '",
                          ...
                          || '",
                                               "N_Cuenta_Mayor": "'
                          || pkg_integra_utal.leerdatotabla_wf(itemtype, itemkey, 'CUENTA', i)
                          || '",
                                               "Centro_Costo": "'
                          || v_centro_costo
                          || '",
                ...
                IF i <> v_contador_centro_costo THEN
                    v_json := v_json || ',';
                END IF;
            END LOOP;
      ...
      /*Fin Json de entrada */
            ...
            */
            ...
            --v_respuesta := call_url_p('http://sappiutalca:piutalca2016@sappoqa.utalca.cl:52000/RESTAdapter/MM004/INT_LEG06', v_json);
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/MM05/INT_LEG06', v_json);
                htp.p(v_respuesta);
...
-- htp.p(v_type);
        INSERT INTO log_wf_licitaciones_sap (
            item_type,
            item_key,
            msg_sap,
            fecha_msg
...
/*
    ...
    PROCEDURE int_leg06 (
        ...
        v_msg           VARCHAR2(5000);
    BEGIN
        l_cli_json := int_leg06_json(itemtype, itemkey, v_ret, v_msg);
    EXCEPTION
        WHEN OTHERS THEN
            htp.p(sqlerrm || dbms_utility.format_error_backtrace); --SQLERRM||DBMS_UTILITY.format_error_backtrace
    ...
    FUNCTION int_sap10_json_interfaces (
        p_cli_rut            VARCHAR2,
...
begin
    ...
    BEGIN
        BEGIN
            l_cli_json := int_sap10_json(p_cli_rut, p_canal_distribucion, v_ret, v_msg);
        EXCEPTION
            WHEN OTHERS THEN
                htp.p(sqlerrm || dbms_utility.format_error_backtrace);
        END;
        ...
        IF v_ret = 'S' THEN
            ...
            htp.p('<br>');
            htp.p('cli_existe_canal_dist:' || lee_json(l_cli_json, 'cli_existe_canal_dist'));
        ELSE
            htp.p(v_msg);
        END IF;
...
--****************************************************************************************************
   ...
   /* Fin Recupera Token*/
        ...
        IF ( p_cli_canal_distribucion = '11' OR p_cli_canal_distribucion = '12' OR p_cli_canal_distribucion = '13' ) THEN
            v_grupo_cliente := '15';
        ELSE
            v_grupo_cliente := '10';
        END IF;
...
--htp.p(v_json);
            ...
            */
                ...
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/SD004/INT_LEG04', v_json);
            ...
            */
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    INSERT INTO log_crea_cliente_sap (
                        rut,
                        mensaje_sql,
                        msg_sap,
                        fecha_msg
...
-- i.villaseca, integracion paga deuda en SAP
    ...
    BEGIN
       ...
       IF p_tipo = 'X' THEN
        ...
        /*select to_char(PACR_FECHA_DEBITO, 'YYYYMMDD') into v_fecha_debito from vec_cob01.pac_rendicion WHERE pacr_num_factura = p_num_op and PACR_ESTADO_DEBITO = '00';*/
    ...
    PROCEDURE int_leg04 (
        p_tipo_interlocutor      VARCHAR2 DEFAULT NULL,
    ...
    BEGIN
     ...
     /*llama a la función int_leg04_json y en la variable l_cli_json recibe el json del data   p_cli_cod_carrera,p_cli_agrupacion ,*/
            ...
            NULL;
            l_cli_json := int_leg04_json(p_tipo_interlocutor, p_cli_rut, 'D000', p_cli_matricula, p_cli_cod_carrera,
                                        p_cli_agrupacion, p_cli_tratamiento, p_cli_nombres1, p_cli_nombres2, p_cli_cod_giro,
                                        p_cli_sexo, p_cli_rubro, p_cli_direccion, p_cli_numero, p_cli_codigo_comuna,
                                        p_cli_region, p_cli_telefono, '', p_cli_email, p_cli_canal_distribucion,
                                        v_ret, v_msg);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
    ...
    FUNCTION int_leg02_json (
        p_codigo_cli        VARCHAR2, --parametro consulta
   ...
   /* Fin Recupera Token*/
    ...
    BEGIN
     /*llama a la función int_leg02_json y en la variable l_cli_json recibe el json del data*/
        l_cli_json := int_leg02_json(p_codigo_cli, p_tipo_documento, p_fecha_documento, p_cuponera, p_documento,
                                    p_cuota, p_fecha_vencimiento, p_monto_local, p_empresa, p_carrera,
                                    p_moneda, p_nro_matricula, v_ret, v_msg);
  ...
  /*imprime estructura json por serpara y se revisa formato*/
        IF v_ret = 'S' THEN
            htp.p('TYPE:'
                  || lee_json(l_cli_json, 'TYPE')
                  || '<br>');
            htp.p('NUMBER:'
                  || lee_json(l_cli_json, 'NUMBER')
                  || '<br>');
            htp.p('MESSAGE:'
                  || lee_json(l_cli_json, 'MESSAGE')
                  || '<br>');
            htp.p('LOG_MSG_NO:'
                  || lee_json(l_cli_json, 'LOG_MSG_NO')
                  || '<br>');
            htp.p('ROW:'
                  || lee_json(l_cli_json, 'ROW')
                  || '<br>');
   ...
   /* Fin Recupera Token*/
      ...
      /*Fin Json de entrada */
           ...
           -- v_respuesta := call_url_p('http://sappiutalca:piutalca2016@sappodev.utalca.cl:51000/RESTAdapter/FISD01/INT_SAP02', v_json);
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/FISD01/INT_SAP02', v_json);
            /*Fin llamada a servicio SAP*/
            EXCEPTION
                WHEN OTHERS THEN
                    p_ret := 'E';
...
--p_msg:=v_respuesta;
    ...
    PROCEDURE int_sap02 (
        p_codigo_cli      VARCHAR2 DEFAULT NULL,
    ...
    BEGIN
     /*llama a la función int_sap02_json y en la variable l_cli_json recibe el json del data*/
        BEGIN
            l_cli_json := int_sap02_json(p_codigo_cli, p_tipo_documento, p_cuenta_contrato, p_fecha_documento, p_documento,
                                        p_cuota, v_ret, v_msg);
        EXCEPTION
            WHEN OTHERS THEN
                htp.p(sqlerrm || dbms_utility.format_error_backtrace);
   ...
   /* Fin Recupera Token*/
      ...
      /*Fin Json de entrada */
            ...
            */
                ...
                v_respuesta := call_url_p(g_sistema_sap || '/RESTAdapter/FI004/INT_SAP11', v_json);
                BEGIN
                    INSERT INTO log_integra_utal (
                        id,
                        llamado,
                        respuesta,
                        fecha
...
/*condor2.utalca.cl/pls/sap_test/pkg_integra_utal_dev.int_sap11?p_codigo_cli=0699617911&p_carrera=&p_nro_matricula=&p_tipo_documento=*/
    PROCEDURE int_sap11 (
        p_codigo_cli     VARCHAR2 DEFAULT NULL,
    ...
    BEGIN
     /*llama a la función int_sap11_json y en la variable l_cli_json recibe el json del data*/
        BEGIN
            l_cli_json := int_sap11_json(p_codigo_cli, p_carrera, p_nro_matricula, p_tipo_documento, p_nro_cupon,
                                        v_ret, v_msg);
        EXCEPTION
            WHEN OTHERS THEN
                htp.p(sqlerrm || dbms_utility.format_error_backtrace);
            ...
            */
            ...
            --     htp.p(g_sistema_sap||'/RESTAdapter/FISD01/INT_SAP02'||'<br>');
                ...
                EXCEPTION
                    WHEN OTHERS THEN
--                 htp.p(SQLERRM||DBMS_UTILITY.format_error_backtrace); --SQLERRM||DBMS_UTILITY.format_error_backtrace
                        pl_mensaje_error := sqlerrm || dbms_utility.format_error_backtrace;
                        INSERT INTO tabla_clob_sap (
                            id,
                            valor
                        ) VALUES (
...
INT_LEG05 -> CREACION DE DEUDA*/
    ...
    FUNCTION int_leg05_fica_crea_mat_col (
        p_idcliente   VARCHAR2,
        p_tipo_doc    VARCHAR2,
...
--
      ...
      --cursor de deudas en tabla temporal que forma el json
        CURSOR c_deudas_actuales_detalle IS
        SELECT
            *
        FROM
            vec_cob02.carga_colchagua a
        WHERE
                interlocutor = p_idcliente
         ...
         /* Fin Recupera Token*/
            ...
            BEGIN
                ...
                FROM
                    operacion_sub_operacion_sap
                WHERE
                    clase_documento_sap = p_tipo_doc;
                ...
                v_line := '{
                    ...
                    "FLAG": "FICA",
                    "BAPI_CTRACDOCUMENT_CREATE": {';
                FOR reg IN c_deudas_actuales_detalle LOOP
                    v_contador := v_contador + 1;
                    v_line := v_line
                              || ' "ZCLFICA_MF_CREADEUDA":{
                               "Codigo_cli": "'
   ...
   /* Fin Recupera Token*/
    ...
    FUNCTION int_leg02_portal_sap2 (
        p_idcliente VARCHAR2,
        p_num_op    VARCHAR2,
        p_ret       OUT VARCHAR2, --Salida estado si tiene error en oracle S (Success) E (Error)
        p_msg       OUT VARCHAR2,
    ...
    BEGIN
        ...
        IF p_ret = 'S' THEN
            ...
            FOR reg_grupo IN c_deudas_actuales(p_idcliente) LOOP
                ...
                htp.p(length(v_json));
                     --LLAMDA A INTEGRACION DE SAP CON EL JSON
                v_respuesta := call_url_p_postgrado(g_sistema_sap || '/RESTAdapter/FI002/INT_LEG02', v_json);
   ...
   /* Fin Recupera Token*/
        ...
        IF p_ret = 'S' THEN
            ...
            BEGIN
                ...
                COMMIT;
                v_respuesta := call_url_p_1(g_sistema_sap || '/RESTAdapter/FICA010/CREA_DESC', v_line_deuda);
            EXCEPTION
                WHEN OTHERS THEN
                    p_ret := 'E';
                    p_msg := p_msg
    ...
    FUNCTION int_crea_descuento_pregrado (
        p_rut_deudor      IN VARCHAR2,
        p_monto_descuento IN VARCHAR2,
        p_codigo_carrera  IN VARCHAR2,
        p_nro_matricula   IN VARCHAR2,
   ...
   /* Fin Recupera Token*/
        ...
        IF p_ret = 'S' THEN
            ...
            BEGIN
                ...
                COMMIT;
                -- v_respuesta := call_url_p_1(g_sistema_sap || '/RESTAdapter/FICA010/CREA_DESC', v_line_deuda);
                v_respuesta := call_url_p_1(g_sistema_sap || '/RESTAdapter/FICA009/CREA_BECA', v_line_deuda); --se cambia integración DS MG J 17/12/2025
            EXCEPTION
                WHEN OTHERS THEN
                    p_ret := 'E';
                    p_msg := p_msg
    ...
    FUNCTION int_crea_descuento_preg (
        p_rut_deudor      IN VARCHAR2,
        p_monto_descuento IN VARCHAR2,
        p_codigo_carrera  IN VARCHAR2,
        p_nro_matricula   IN VARCHAR2,
    ...
    BEGIN
        p_ret := 'S';
        v_fecha_inicio := TO_CHAR(TO_DATE('31-MAR-' || EXTRACT(YEAR FROM SYSDATE), 'DD-MON-YYYY'), 'YYYYMMDD');
        v_fecha_fin := TO_CHAR(TO_DATE('31-DIC-' || EXTRACT(YEAR FROM SYSDATE), 'DD-MON-YYYY'), 'YYYYMMDD');
   ...
   /* Fin Recupera Token*/
        ...
        IF p_ret = 'S' THEN
            ...
            BEGIN
                ...
                COMMIT;
                v_respuesta := call_url_p_1(g_sistema_sap || '/RESTAdapter/FICA009/CREA_BECA', v_line_deuda);
            EXCEPTION
                WHEN OTHERS THEN
                    p_ret := 'E';
                    p_msg := p_msg
    ...
    FUNCTION int_crea_beca_postgrado (
        p_tipo_proceso        IN VARCHAR2,
        p_rut_deudor          IN VARCHAR2,
        p_clase_documento_sap IN VARCHAR2,
        p_monto_descuento     IN VARCHAR2,
               ...
               --obtenemos el material de la tabla de productos
                        ...
                        BEGIN
                            ...
                            FROM
                                operacion_sub_operacion_sap
                            WHERE
                                clase_documento_icon = reg_grupo.pade_tipo_documento;
                        ...
                        BEGIN
                            ...
                            FROM
                                vec_cob01.pop_productos       p,
                                vec_cob01.pop_productos_items sp
                            WHERE
                                    p.prod_id = sp.prod_id
                ...
                -- de lo contrario toma el general de la clase de producto .
                        ...
                        l_centro_gestor_base := nvl(l_centro_gestor_base2, l_centro_gestor_base);
                        IF reg.pade_tipo_cliente = 'P' THEN
                            l_observacion_subproducto := l_observacion_subproducto
                                                         || '- '
                                                         || reg.pade_prod_nomcliente;
                        ELSE
                            l_observacion_subproducto := l_observacion_subproducto || reg.pade_observacion;
                        END IF;

                        IF nvl(reg.pade_tipo_cliente, 'R') = 'R' THEN
                            l_id_sap := reg.pa_rut;
                            l_cli_matricula := p_idcliente;
                        ELSE
                 --   l_id_Sap := vec_cob02.cli_extranjero_pasaporte_pkg.id_sap2(reg.pade_prod_idcliente );
                    ---l_cli_matricula := l_id_Sap ;
                        ...
                        v_line := v_line
                               ...
                               "Sub_operacion": "'
                                  ...
                                  || '",
                               "Descripcion": "'
                                  || substr(replace(l_observacion_subproducto || reg.pade_observacion, '"', ''), 1, 47)
                                  || '@'
                                  || v_convenio
                                  || '",
                               "Elemento_PEP": "'
