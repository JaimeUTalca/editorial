       ...
       /*llama a la función int_leg04_json*/
                BEGIN
                    ...
                    ,
                                                                           ...
                                                                           ,
                                                                           v_ret, v_msg);
/*ariquelme 17102025 comentar linea de codigo l_data_json2*/
                   l_data_json2 :=
                        JSON(
                            l_cli_json2.get('data')
                        );
    ...
    --CREACION DE CLIENTE, SI NO ES EXTRANJERO
        ...
        --AVANZAMOS EL WF DE TITULACION
            ...
            ELSIF pl_modulo_origen IN ( 'PORTALMATRICULA' ) THEN
                ...
                /*SE ACTUALIZA DESDE PRODUCCION DEJANDO SIN EFECTO EL BLOQUE COMENTADO. 08-01-2026 --- AROJAS*/
                /*BEGIN
                ...
                END IF;*/
                /*ACTUALIZACION DESDE PROD*/
                 if nvl(
                          p_visualiza_log,
                          'N'
                       ) = 'S' then
                        if p_opcion = 'GDSP' then
                         --GENERAR DEUDA Y PAGAR
                            l_cli_json := utsap001.pkg_integra_utal.int_leg05_fica_matricula_sap(
                                                                                                tbk_orden_compra,
                                                                                                tbk_id_sesion,
                                                                                                v_ret,
                                                                                                v_msg
                                          );
                          -- OBTIENE DESCUENTO SEGÚN DECIL DEL ALUMNO
                            v_desc     := sdi_sgc.obtiene_descuento_decil(
                                                                     tbk_orden_compra,
                                                                     to_char(
                                                                            sysdate,
                                                                            'YYYY'
                         ...
                         -- CARGA DESCUENTO EN SAP
                            if v_desc > 0 then
                                /*l_cli_json3 := utsap001.pkg_integra_utal.int_crea_descuento_pregrado(
                                                                                                    tbk_orden_compra,
                                                                                                    v_desc,
                                                                                                    reg.pade_nro_carrera,
                                                                                                    reg.pade_matricula,
                         ...
                         --SOLO PAGAR
                            l_cli_json := utsap001.pkg_integra_utal.int_leg02_portal_sap2(
                                                                                         tbk_orden_compra,
                                                                                         tbk_id_sesion,
                                                                                         v_ret,
                                                                                         v_msg
            ...
            ELSIF pl_modulo_origen IN ( 'PORTALPREVENTAS' ) THEN
                ...
                */
                ...
                /* JV: Fin PORTALPREVENTAS */
                    ...
                    if ( p_opcion = 'SP' ) then
                        l_cli_json := utsap001.pkg_integra_utal.int_leg02_portal_sap(
                                                                                    tbk_orden_compra,
                                                                                    tbk_id_sesion,
                                                                                    v_ret,
                                                                                    v_msg
                        ...
                        begin
                            l_cli_json := utsap001.pkg_integra_utal.int_leg05_sd_venta(
                                                                                      tbk_orden_compra,
                                                                                      tbk_id_sesion,
                                                                                      v_ret,
                                                                                      v_msg
                        ...
                        begin
                            select a.pade_nro_documento
                              into v_id_venta
                              from vec_cob01.pop_pagos_detalle_temp_sap a
                             where pa_rut = tbk_orden_compra
                               and pa_nro_operacion = tbk_id_sesion;
                        --update
                            update vec_cob03.pove_venta_transacciones
                               set
                                vetr_estado = 1
                             where vetr_codigo = v_id_venta;
                        ...
                        --ENVIO DE EMAIL
                            ...
                            --V_RESP_EMAIL:=get_url_file('http://portaldepagos.utalca.cl/portaleditorial/envio_correo_editorial.php?id_venta='||v_id_venta||'&orden_compra='||TBK_ID_SESION);
                                vec_cob03.venta_online.envio_correo_ingresar(
                                                                            v_id_venta,
                                                                            tbk_id_sesion
                                );
                            exception
